...
class Testナップサック {
    ...
    static int[] ナップサック(int maxSize, int[] sizes, int[] values) {
        ...
        // 全体の単一制約
        // p.constraint(q -> IntStream.range(0, size).map(i -> q[i] * sizes[i]).sum() <= maxSize, variables);
        // 分割した制約
     ...
     *
     * 品物 1個あたりの価値 1個あたりのサイズ 缶コーヒー 120 10 水入りペットボトル 130 12 バナナ 80 7 りんご 100 9
     * おにぎり 250 21 パン 185 16
     */
    @Test
    void test() {
        int maxSize = 65;
